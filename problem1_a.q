...
.aq.q0:{[]
 ...
 .aq.t1:?[trade;();.aq.grpexpr:(enlist[`x__0]!enlist {x^.aq.cd x}`stocksymbol);({x!x}cols trade)];
 // shadowing columns used as named groups, if want original, use correlation name
 .aq.cd:() _ .aq.cd;
 .aq.t1:?[.aq.t1;();0b; .aq.acctgrps[ ; .aq.grpexpr](enlist[`stocksymbol]!enlist {x^.aq.cd x}`stocksymbol),(enlist[`average_price]!enlist (%;(sum';(*;{x^.aq.cd x}`quantity;({x^.aq.cd x}`trade.price)));(sum';{x^.aq.cd x}`quantity)))];
 .aq.t1
 }
.aq.q0[]
