...
.aq.q0:{[]
 ...
 .aq.cd:{(key[y] inter .aq.pc) _ x,y}[.aq.cd;.aq.gencd[trade;"trade";0b]]; //drop ambiguous cols from map
 .aq.pc:.aq.pc union cols trade; //store orig col names to avoid collisions
 .aq.six:?[trade;();();(iasc;{x^.aq.cd x}`time)];
 .aq.t1:![trade;();0b;c!( ;.aq.six) each c:(),{x^.aq.cd x}`quantity`price`stocksymbol];
 .aq.t1:?[.aq.t1;();.aq.grpexpr:(enlist[`x__0]!enlist {x^.aq.cd x}`stocksymbol);({x!x}cols .aq.t1)];
 // shadowing columns used as named groups, if want original, use correlation name
 .aq.cd:() _ .aq.cd;
 .aq.t1:?[.aq.t1;();0b; .aq.acctgrps[ ; .aq.grpexpr](enlist[`stocksymbol]!enlist {x^.aq.cd x}`stocksymbol),(enlist[`weighted_moving_avg]!enlist (%;(mavg';10;(*;{x^.aq.cd x}`price;{x^.aq.cd x}`quantity));(mavg';10;{x^.aq.cd x}`quantity)))];
 .aq.t1
 }
.aq.q0[]
